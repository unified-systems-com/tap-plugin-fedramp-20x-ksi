...
- Columns (left-to-right):
  ...
  - **System** — the parent asset's name. Rendered as a link to the standard system page (`/grid/<entity_id>`) via a Tabulator formatter. Cell shows `—` when the finding has no `CARRIES_COMPLIANCE_FINDING` parent.
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-ksi-profile-findings-table-1 | Two-Hop Gryphon | Implemented | The findings table is loaded by a two-hop gryphon query (`CONCERNS_COMPLIANCE_CONTROL` ← finding ← `CARRIES_COMPLIANCE_FINDING`) returning flat (system, finding) row pairs. | |
| req-ksi-profile-findings-table-2 | Per-System Rows | Implemented | A finding with multiple `CARRIES_COMPLIANCE_FINDING` parents emits one row per parent. Findings without any parent are not surfaced in v0 (gryphon parser limitation; tracked in Future Work). | |
...
- **Orphan findings in the findings table**: Once gryphon supports `OPTIONAL MATCH` (or once a small fallback module pass is added), surface findings that lack any `CARRIES_COMPLIANCE_FINDING` parent. Until then, the inner-joined two-hop in `req-ksi-profile-findings-table` only shows findings with at least one system parent.
